(function ($) {
      ...
      if (!$formContainer.find('.original-selector').val()) {
        ...
        if (emptySections.length) {
          emptySections.forEach(el => {
            ...
            labelPrefix.innerText = 'please select size';
  ...
  theme.applyAjaxToProductForm = function ($formContainer) {
    ...
    //             // 显示购物车抽屉
    //             if ($('body').toggleClass('show-drawer-cart').hasClass('show-drawer-cart')) {
    //                 $('.main-drawer-cart-container .lazyload--manual').removeClass('lazyload--manual').addClass('lazyload');
    //             }

    //             // 购物车抽屉 Ajax 产品列表
    //             return getCartData(cartData),false;  
    ...
    theme.LoadFilterer = function (section) {
      ...
      this.functions = {
        ...
        ajaxLoadFilterForm: function ajaxLoadForm(evt) {
          if (evt.type === 'submit') {
            evt.preventDefault();
          }
          var queryVals = [];
          ...
          newUrl = newUrl.replace('&', '?');
          // load
          this.functions.ajaxLoadFilterUrl.call(this, newUrl);
        },
        ...
        ajaxLoadUrl: function ajaxLoadUrl(url) {
          ...
          this.currentAjaxLoadUrlFetch = $.get(url, function (data) {
            ...
            var filterResult = document.querySelector('.filter-group--applied');
            var newWidth = 100 * filterResult.offsetWidth / window.innerWidth;
            console.log(filterResult.offsetWidth);
            console.log(newWidth);
        
            if (newWidth >= 65.3125) {
              filterResult.classList.add('overflow-x-scroll');
            }

            const box = document.querySelector('.filter-group--applied');
        ...
        ajaxLoadFilterUrl: function ajaxLoadUrl(url) {
          // update url history
          var fullUrl = url;
          if (fullUrl.slice(0, 1) === '/') {
            fullUrl = window.location.protocol + '//' + window.location.host + fullUrl;
        ...
        ajaxSubmitSP: function ajaxSubmit(url) {
          // start fetching URL
          var refreshContainerSelector = '[data-ajax-container]',
          $ajaxContainers = this.$container.find(refreshContainerSelector);
          ...
          this.currentAjaxLoadUrlFetch = $.get(fullUrl, function (data) {
            this.currentAjaxLoadUrlFetch = null;
      ...
      if (this.$container.data('ajax-filtering')) {
        ...
        this.$container.on('change' + this.namespace + ' submit' + this.namespace, '#CollectionFilterForm', this.functions.ajaxLoadFilterForm.bind(this));
        // handle back button
        this.registerEventListener(window, 'popstate', this.functions.ajaxPopState.bind(this));
        this.$container.on('click' + this.namespace, '.submit-filter', this.functions.ajaxSubmitSP.bind(this));
      } else {
    ...
    theme.buildGalleryViewer = function (config) {
      ...
      $allContainer.on('click.galleryViewer select.galleryViewer', '.gallery-viewer__thumb', function (evt) {
        ...
        $currentZoomImage.on('load', function () {
          ...
          setCurrentTransform(0, 0, .2); // centre, zoomed in
    ...
    theme.Navigation = {
      ...
      // 2022/06/27 修改
        ...
        $mobileDrawer.on('click', '.navigation__tier-1 > .navigation__item > .navigation__children-toggle', function (evt) {
          evt.preventDefault();
          var tit = $(this).parent('.navigation__item--with-children');
           if(tit.hasClass("toggle_on")){
              $(tit).removeClass("toggle_on");
              $(tit).find('.navigation__tier-2-container').slideUp();
          }
          else{
              $(tit).addClass("toggle_on");
              $(tit).find('.navigation__tier-2-container').slideDown();
          };
          ...
          // scroll container
          tit.find('.navigation__tier-2 .navigation__item').addClass('navigation__item--open');
        ...
        // hover events
        $nav.on('mouseenter mouseleavec click', '.navigation__tier-1 > .navigation__item--with-children', function (evt) {
          ...
          // a11y
          $dropdownContainer.children('[aria-expanded]').attr('aria-expanded', evt.type == 'click');
        });
    ...
    theme.ProductBlockManager = new function () {
      ...
      _.monitorSwatchSelection = function (container) {
        if ($('[data-media].product-block-options__item', container).length) {
          $(container).on('click.productBlockManager click.product-block-options__item', '[data-media].product-block-options__item', function () {
            var $images = $(this).closest('.product-block').find('.product-block__image');
      ...
      _.initProductOptions = function ($productForm) {
        ...
        $productForm.find(_.selectors.primaryVariantIdInput).each(function () {
          ...
          $(this).on('change.themeProductOptions firstrun.themeProductOptions', function (e) {
            ...
            var dummyVariant = null;
            var dummyVariantId = null;
            if(!variant){
              var selectedColorEl = document.querySelector("body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--swatch .opt-btn:checked");
              var selectedColor = selectedColorEl.getAttribute("value");
              var optionWithSelectedColorEl = document.querySelector("select option[data-color='" + selectedColor + "']");
              if(optionWithSelectedColorEl){
                dummyVariantId = optionWithSelectedColorEl.getAttribute("value");
              }
            }

            if(dummyVariantId){
              for (var i = 0; i < productData.variants.length; i++) {
                if (productData.variants[i].id == dummyVariantId) {
                  dummyVariant = productData.variants[i];
                }
              }
            }
            ...
            if (variant && variant.featured_media) {
              ...
              if(dummyVariant){
                $container.find(_.selectors.gallery).trigger('variantImageSelected', [dummyVariant, e.type === 'firstrun']);
              }
            }
        ...
        if (pageWidth > 767) {
          ...
          if ($block.toggleClass('expanded').hasClass('expanded')) {
            ...
            theme.currentQuickbuyRequest = $.get(url, function (response) {
              ...
              $newDetail.find('.gallery .thumbnails').removeClass('mobile-only-important');
    ...
    theme.HeaderSection = new function () {
      this.onSectionLoad = function (container) {
        ...
        // Toggle 购物车抽屉
        $(container).on('click.headerSection', '.show-cart-drawer-link', function (e) { 
          if ($('body').toggleClass('show-drawer-cart').hasClass('show-drawer-cart')) { 
            $('.main-drawer-cart-container .lazyload--manual').removeClass('lazyload--manual').addClass('lazyload');
          } else {
            $('.show-cart-drawer-link:first').focus();
          }
          return false;
        }); 
    ...
    //抽屉   
    $('body').on('click', '.main-drawer-cart .drawer-cart-header-bar .drawer-cart-header-bar-close, .main-drawer-cart .main-drawer-cart-mask-close', function (e) {  
      if ($('body').toggleClass('show-drawer-cart').hasClass('show-drawer-cart')) { 
        $('.main-drawer-cart-container .lazyload--manual').removeClass('lazyload--manual').addClass('lazyload');
      } else {
        $('.show-cart-drawer-link:first').focus();
    ...
    $(function ($) {
      ...
      $(document).on('load-product-form', '.product-form', function (e) {
        ...
        var $iconOptionContainers = $productDetail.find('.option-selector--swatch'),
            $iconOptionContainers_size = $productDetail.find('.option-selector--with-size-chart');
        ...
        if ($iconOptionContainers_size) {
          $iconOptionContainers_size.find('.label').append('<span class="label__value">');
          let flag = 0;
          $(this).on('variantChanged', function (evt, variant) {
            $iconOptionContainers_size.each(function () {
              var optionIndex = $(this).data('option-index');
              if (variant) {
                var optionValue = variant.options[optionIndex];
                if (flag == 0) {
                  flag = flag + 1;
                  $(this).find('.label').append('<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"><g id="arrow_next"><path id="Vector" d="M1.31687 5.66113L6.99999 8.88543L12.6831 5.66113" stroke="#282828" stroke-width="0.56" stroke-miterlimit="10"/></g></svg>');
                } else {
                  if (document.querySelector('body.template-product .variant-visibility-area .lightly-spaced-row')) {
                    document.querySelector('body.template-product .variant-visibility-area .lightly-spaced-row').classList.remove('visible-hidden');
                  }
                  $(this).find('.label').text(optionValue);
                  $(this).find('.label').append('<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"><g id="arrow_next"><path id="Vector" d="M1.31687 5.66113L6.99999 8.88543L12.6831 5.66113" stroke="#282828" stroke-width="0.56" stroke-miterlimit="10"/></g></svg>');
                }
                var element = document.querySelector("body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--with-size-chart .option-selector__btns");
                ...
                if (selectedOption) {
                  if (flag = 0) {
                    ...
                    $(this).find('.label').text(selectedOption.value);
                    $(this).find('.label').append('<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"><g id="arrow_next"><path id="Vector" d="M1.31687 5.66113L6.99999 8.88543L12.6831 5.66113" stroke="#282828" stroke-width="0.56" stroke-miterlimit="10"/></g></svg>');
                    flag = flag + 1;
                  }
                  var element = document.querySelector("body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--with-size-chart .option-selector__btns");
              ...
              Object.keys(optionStatusMap[_optionIndex].values).forEach(optionValue => {
                ...
                for (var _i3 = 0; _i3 < productData.variants.length; _i3++) {
                  ...
                  for (var j = 0; j < fixedOptionsForValue.length; j++) {
                    if (fixedOptionsForValue[j] && fixedOptionsForValue[j] !== _v.options[j]) {
      ...
      $(document).on('click', '.quantity-wrapper [data-quantity]', function () {
        ...
        let minusBtn = document.querySelector('.data-quantity-btn--minus');
        if ($qty.val() == '1') {
          minusBtn.classList.add('button-disable')
        } else {
          minusBtn.classList.remove('button-disable')
        }
        return false;
      });
      ...
      theme.Sections.register('page-list-collections-template', theme.ListCollectionsTemplateSection, { deferredLoad: false });
       ...
       theme.Sections.register('cookies-popup', theme.CookiesPopup);
    });
  ...
  $('#r--ryviu-theme').owlCarousel({
          items: 3
      });
  $(".section-footer__title").click(function(){
    if($(this).parents(".section-footer__menu-block").hasClass("ondown")){
      $(this).parents(".section-footer__menu-block").removeClass("ondown");
      
       $(this).parents(".section-footer__menu-block").find(".section-footer__menu-block__menu").slideUp();
    }
    else{
      $(this).parents(".section-footer__menu-block").addClass("ondown");
      $(this).parents(".section-footer__menu-block").find(".section-footer__menu-block__menu").slideDown();
    }
    ...
    var getCartData = function(data){
        ...
        var cartDataListList = cartData.items;
        var cartTotalPrice = theme.Shopify.formatMoney(cartData.total_price, theme.money_format);
        var cartItemCount = cartData.item_count;
 
        console.log('Debug Adny Hus');
        console.log(cartData);

        if(cartData.items.length>0){
            $('.show-drawer-cart #drawer-cart-product-form').css("display", "block"); 
            $('.show-drawer-cart #drawer-empty-cart').css("display", "none"); 
            var cartDataList = '';
            for (var i = 0; i < cartDataListList.length; i++) {
                let Id = cartDataListList[i]['id'];
                let productImage = cartDataListList[i]['image'];
                let productTitle = cartDataListList[i]['title'];
                let productQuantity = cartDataListList[i]['quantity'];
                let productPrice = theme.Shopify.formatMoney(cartDataListList[i]['final_price'], theme.money_format);

                cartDataList += '<div class="drawer-cart-product-item">'+
                                '<div class="drawer-cart-product-item-thumb"><img src="'+productImage+'" alt="'+productTitle+'" height="180" class="swiper-lazy"/></div>'+
                                '<div class="drawer-cart-product-item-info">'+
                                '<div class="drawer-cart-product-item-info-title">'+productTitle+'</div>'+
                                '<div class="drawer-cart-product-item-info-price">'+productPrice+'</div>'+
                                '<div class="drawer-cart-product-item-info-quantity">'+
                                '<div class="drawer-cart-product-item-info-quantity-input">'+
                                '<div class="notabutton quantity-dec" onclick="decCartProduct('+Id+','+productQuantity+')"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="feather feather-minus"><title>{{ \'general.icon_labels.minus\' | t }}</title><line x1="5" y1="12" x2="19" y2="12"></line></svg></div>'+
                                '<div class="quantity-input"><input class="quantity-input-input" type="number" name="product-quantity" value="'+productQuantity+'"/></div>'+
                                '<div class="notabutton quantity-inc" onclick="incCartProduct('+Id+','+productQuantity+')"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><title>{{\'general.icon_labels.plus\' | t }}</title><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></div></div><div class="notabutton quantity-delete" onclick="removeCartProduct('+Id+')"><?xml version="1.0" standalone="no"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd"><svg t="1669077322453" class="icon" viewBox="0 0 1088 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4134" xmlns:xlink="http://www.w3.org/1999/xlink" width="911.625" height="858"><path d="M870.4 243.2h-134.4V166.4c0-57.6-44.8-102.4-102.4-102.4H486.4C428.8 64 384 108.8 384 166.4v76.8H249.6c-25.6 0-44.8 19.2-44.8 44.8s19.2 44.8 44.8 44.8h614.4c25.6 0 44.8-19.2 44.8-44.8s-19.2-44.8-38.4-44.8z m-224 0H473.6V166.4c0-6.4 6.4-12.8 12.8-12.8h147.2c6.4 0 12.8 6.4 12.8 12.8v76.8zM812.8 422.4c-25.6 0-44.8 12.8-51.2 38.4l-38.4 396.8c0 6.4-6.4 12.8-12.8 12.8H409.6c-6.4 0-12.8-6.4-12.8-12.8l-44.8-396.8c0-25.6-25.6-44.8-51.2-38.4s-38.4 25.6-32 51.2l38.4 396.8c6.4 51.2 44.8 89.6 102.4 89.6h307.2c51.2 0 96-38.4 102.4-96l38.4-396.8c0-19.2-19.2-44.8-44.8-44.8z" fill="#8C8C8C" p-id="4135"></path></svg></div>'+
                                '</div></div></div>';
            }
        }else{
            cartDataList = '';
            $('.show-drawer-cart #drawer-cart-product-form').css("display", "none"); 
            $('.show-drawer-cart #drawer-empty-cart').css("display", "block"); 
        }

        //列出购物车产品列表
        $('.show-drawer-cart #drawer-cart-product-container').html(cartDataList);
        $('.show-drawer-cart #cart-total_price').text(cartTotalPrice); 
        $('.cart-link .cart-link__count').text(cartItemCount); 

        
    } 

    //
    var removeCartProduct = function(product_id){
        let handle = confirm("Are you sure to delete this product?");
        if(handle==true)
        {  
            jQuery.post(theme.routes.cart_url + '/update.js', {
                updates: {
                    [product_id]: 0
                }
              }, function(res){
                  return getCartData(res);
              }
            );
        }
        else
    ...
    //
    var decCartProduct = function(product_id, curNum){
        let num = parseInt(curNum - 1);
        jQuery.post(theme.routes.cart_url + '/update.js', {
            updates: {
                [product_id]: num
            }
    ...
    //
    var incCartProduct = function(product_id, curNum){
        let num = parseInt(curNum + 1);
        jQuery.post(theme.routes.cart_url + '/update.js', {
            updates: {
                [product_id]: num
    ...
    const btnLegend = document.querySelector('body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--with-size-chart legend.label');
    if (btnLegend) {
      btnLegend.onclick = function() {
        var element = document.querySelector("body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--with-size-chart .option-selector__btns");
        element.classList.toggle("visible-hidden");
      }
    }

    function onChangeQuantity() {
      const btnMinusQuantity = document.querySelector('.data-quantity-btn--minus');
      const inputQuantity = document.querySelector('.quantity-wrapper input[name=quantity]').value;
      const tesstt = document.querySelector('.quantity-wrapper input[name=quantity]');
      tesstt.value = tesstt.value.replace(/[^1-9]/g, '').replace(/(\..*?)\..*/g, '$1');
      if ( (inputQuantity > 1) && (btnMinusQuantity.classList.contains('button-disable')) ) {
        btnMinusQuantity.classList.remove('button-disable')
      }
      else if ((inputQuantity <= 1) && !(btnMinusQuantity.classList.contains('button-disable'))) {
        btnMinusQuantity.classList.add('button-disable')
      }
    }
        ...
        for (const colorItem of color) {
          if (colorItem.checked == true) {
            ...
            for (const sizeItem of size) {
              if (sizeItem.checked == true) {
                error_ms.innerText = "";
                if (btnSubmit.innerText == 'SOLD OUT') {
                  error_ms.innerText = errors_message_product_out_of_stock;
                  break;
                } else {
                  error_ms.innerText = "";
                  btnSubmit.click();
                ...
                error_ms.innerText = errors_message_size_when_add_to_cart;
              }
            }
            break;
          } else {
            error_ms.innerText = errors_message_color_when_add_to_cart;
          }
        }
      } else {
        btnSubmit.click();
    ...
    addEventListener("load", () => {
      var sw = document.querySelector('.option-selector--swatch'),
      size = document.querySelector('.option-selector--with-size-chart'),
      optionSize = document.querySelectorAll('.option-selector--with-size-chart .opt-btn'),
      optionSizeVal = document.querySelectorAll('.option-selector--with-size-chart .opt-btn-btn input');


      if (sw) {
      }else {
        size.classList.add('size-only');
        optionSize.forEach((e, index) => {
          if(index==0){
            e.click();
          }

          e.onclick = function () {
            size.classList.remove('size-only');
            var element = document.querySelector("body.template-product .section-main-product .product-detail .product-form .option-selectors .option-selector--with-size-chart .option-selector__btns");
            element.classList.toggle("visible-hidden");
          }
        })
      }
      if (size && !size.classList.contains('size-only')) {
        document.querySelector('.option-selector--with-size-chart legend.label span').innerText = 'Size';
        optionSizeVal.forEach((val) => {
          val.checked = false;
        })

        const selectorBtn = document.querySelector('.size-wrapper .label.justify-between');
        const sizeOptions = document.querySelector('.size-wrapper .option-selector__btns');
  
        document.addEventListener('click', (event) => {
          if (!selectorBtn.contains(event.target)) {
            sizeOptions.classList.add("visible-hidden");
          }
        })
      }
